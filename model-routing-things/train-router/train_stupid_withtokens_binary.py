...
PATH_TO_TRAIN_DATASET = "/home/sophiapi/model-routing/OpenHands/evaluation/evaluation_outputs/datasets/20250822_011202_router_train_model-5_instance-100_pruned-4_with-ids_by-example_sample800_balanced-40-60_max-length-4096.jsonl"
PATH_TO_VALID_DATASET = "/home/sophiapi/model-routing/OpenHands/evaluation/evaluation_outputs/datasets/20250822_011509_router_valid_model-5_instance-100_pruned-4_with-ids_by-example_sample200_balanced-40-60_max-length-4096.jsonl"
...
CHECKPOINT_DIR = f"/data/user_data/sophiapi/checkpoints/stupid_withtokens_qwen3_router_model-5_instance-100_pruned-4_with-ids_by-example_balanced-{int(SUCCESS_RATIO*100)}-{int(FAILURE_RATIO*100)}_max-length-{MAX_TOKENS}_binary"
WANDB_NAME = f"qwen3-router-sft-stupid-withtokens-model-5-instance-100-pruned-4-with-ids-by-example-balanced-{int(SUCCESS_RATIO*100)}-{int(FAILURE_RATIO*100)}-max-length-{MAX_TOKENS}_binary"
...
# Set the yes and no token ids
YES_TOKEN_ID = tokenizer.convert_tokens_to_ids("[YES]")
NO_TOKEN_ID = tokenizer.convert_tokens_to_ids("[NO]")
print(f"YES token id: {YES_TOKEN_ID}")
print(f"NO token id: {NO_TOKEN_ID}")

# If the yes and no token ids are the same, throw an error
if YES_TOKEN_ID == NO_TOKEN_ID:
    raise ValueError("YES and NO token ids are the same")
...
class BinaryChoiceSFTTrainer(SFTTrainer):  
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):  
        ...
        # Extract only [YES]/[NO] token logits  
        yes_no_logits = logits[:, :, [self.yes_token_id, self.no_token_id]]  
          
        # Create binary labels (0 for [YES], 1 for [NO])  
        binary_labels = torch.where(  
            labels == self.yes_token_id, 0,  
            torch.where(labels == self.no_token_id, 1, -100)  
        )  
          
        # Compute binary cross-entropy loss  
        loss = F.cross_entropy(  
            yes_no_logits.view(-1, 2),   
...
# Initialize trainer  
trainer = BinaryChoiceSFTTrainer(  
